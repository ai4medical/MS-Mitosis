...
class DataEncoder:
    ...
    def _get_anchor_boxes(self, input_size):
        ...
        num_fms = len(self.anchor_areas)
        fm_sizes = [
            (input_size / pow(2.0, i + 3)).ceil() for i in range(num_fms)
    ...
    def encode(self, boxes, labels, input_size):
        ...
        if len(boxes) == 0:
            boxes = torch.tensor([[128, 128, 256, 256]])
        ...
        # print(torch.unique(cls_targets))
    ...
    def decode(self, loc_preds, cls_preds, input_size, CLS_THRESH=0.5, NMS_THRESH=0.5):
        ...
        ids = ids.nonzero().flatten()
        # print(f"boxes: {boxes.shape}")
        # print(f"id: {ids.shape}")
        try:
